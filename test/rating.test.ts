...
test('isRatingOnGrid accepts 0..5 quarter steps', () => {
  for (const value of [0, 0.25, 1, 1.25, 3, 3.5, 3.75, 4.25, 4.75, 5]) {
...
test('isRatingOnGrid rejects off-grid and out-of-range values', () => {
  for (const value of [-0.25, 5.25, 3.1, 3.3, 3.7, 3.6, 3.05]) {
...
test('checkInRatingSchema parses quarter-step values and rejects the rest', () => {
  ...
  assert.equal(checkInRatingSchema.safeParse(0).success, true);
  ...
  assert.equal(checkInRatingSchema.safeParse(-0.25).success, false);
